...
class SystemFacade {
    ...
    public function __construct($project) {
        $this->_uploadPath= FCPATH .'assets\uploads\\';
    ...
    public function makeFileList(){
        ...
        if($this->_currentFile != $this->_fileSystem->getRoot()->getID()){
            $htmlCode.='<li id="'.$this->_crypter->Encode($this->_fileSystem->getDirParent($this->_currentFile)->getID()).'" class="SystemDirectory return"><img src="'.base_url('/assets/img/return.gif').'"/> ...</li>';
        }
        foreach($this->_fileSystem->getDirContent($this->_currentFile) as $object){
            ...
            if(get_class($object)!='SystemDirectory'){
                $htmlCode .= '<li id="'.  $this->_crypter->Encode($object->getID()).'" class="'.get_class($object).' file"><img class="fileIcone pull-left" src="'.$object->getImage().'"/><div class="fileName listElement">'.$object->getVirtualName().'</div><div class="fileDate listElement" >'.date_format($date, 'd/m/y').'</div>'.'<img src="'.base_url('/assets/img/page_down.gif').'" class="dlImage" title="Download"/></li>';  
            }
            else{
                $htmlCode .= '<li id="'.  $this->_crypter->Encode($object->getID()).'" class="'.get_class($object).' "><img class="fileIcone pull-left" src="'.$object->getImage().'"/><div class="fileName listElement">'.$object->getVirtualName().'</div><div class="fileDate listElement" >'.date_format($date, 'd/m/y').'</div><img src="'.base_url('/assets/img/page_down.gif').'" class="dlImage" title="Download"/></li>';  
