...
class Displayer {
    ...
    private function displayImage(){
        echo '<img id="displayerImg" src="../'.str_replace('\\','/',str_replace(FCPATH, '', $this->file->getSource())).'"/>';
    }
    ...
    private function displayVideo(){
        ...
        if($extension == 'mp4'){
            echo '<video controls><source src="../'.str_replace('\\','/',str_replace(FCPATH, '', $this->file->getSource())).'" type="video/mp4" /></video>';
        }
        else{
    ...
    private function displayAudio(){
        $code.='<audio controls>
                    <source src="../'.str_replace('\\','/',str_replace(FCPATH, '', $this->file->getSource())).'" />
                </audio>';
        echo $code;
    }
}
