...
var ConversationManager = function(userId){
    ...
    this.createConversation = function(conversationUsers){
        ...
        sqlConnection.query('INSERT INTO conversation (id, participant) VALUES ("","'+userList+'")',
    ...
    this.newRoom = function(idConverse, usersId){
        ...
        console.log(usersId);
        usersId = typeof (usersId) == 'array' ? usersId : this.parseIdList(usersId);
    ...
    this.getConverseNotif = function (){ //recuperer les conversations dont le dernier message date de quand l'utilisateur etait deco
        ...
        function(error, result){
            ...
            sqlConnection.query('SELECT * FROM ( SELECT DISTINCT id_conversation, date_message FROM message AS M JOIN conversation AS C ON M.id_conversation=C.id WHERE participant LIKE "%& '+ userId +' &%" ORDER BY id_conversation, date_message DESC ) AS T GROUP BY T.id_conversation',
                function(error, results){
                    if(error){
                        console.log(error);
                        return;
